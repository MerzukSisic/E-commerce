...
export class AddressPipe implements PipeTransform {
  ...
  transform(value?: ConfirmationToken['shipping'] | ShippingAddress | Address, ...args: unknown[]): unknown {
    if (value && 'address' in value && value.name) {
      ...
      const name = 'name' in value ? (value as ShippingAddress).name : '';
      const {line1, line2, city, state, country, postalCode} = value as ShippingAddress;
      return `${name ? name + ', ' : ''}${line1}${line2 ? ', ' + line2 : ''}, ${city}, ${state}, ${postalCode}, ${country}`;
    } else if (value && 'line1' in value) {
      const {line1, line2, city, state, country, postalCode} = value as Address;
