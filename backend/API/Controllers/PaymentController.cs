    ...
    : BaseApiController
{
    ...
    public async Task<IActionResult> StripeWebhook()
    {
        ...
        try
        {
            ...
            await HandlePaymentIntentSuceeded(intent);
            return Ok();
        }
    ...
    private async Task HandlePaymentIntentSuceeded(PaymentIntent intent)
    {
       ...
       if (Math.Round(order.GetTotal() * 100) != intent.Amount)
...
;        }
        catch (Exception ex)
        {
           ...
           throw new StripeException("Invalid signature detected", ex);
